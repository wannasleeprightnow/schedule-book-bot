...
from services.parse_schedule import ParsedSchedule, ScheduleExcelParser
from services.schedule import get_schedule


async def is_new_schedule_exists() -> None:
    while True:
        try:
...
async def new_schedule(parser: ScheduleExcelParser) -> None:
    ...
    for telegram_user_id in all_users_telegram_ids:
        ...
        await bot.send_message(
            telegram_user_id,
            f"{hbold(f'Обновление расписания на \n{formated_date}')}\n\n{
                schedule
            }",
    ...
    for one_grade_schedule in schedules:
        ...
        lessons = ", ".join(
            list(
                map(
                    str,
                    await database.get_lessons_ids_by_input_name(
...
async def divide_books(
    ...
    lessons = list(
        map(
            str,
            (
                await database.get_lessons_ids(telegram_user_id, lessons_date)
            ).split(", "),
        )
    )
